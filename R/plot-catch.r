plot_catch <- function(modlst,themod,obspred = FALSE){
  ...
  ggplot(df,aes(x=Year,y=Catches,color=type)) + geom_line(linewidth=2) + theme_few() + ylab("Catches (t)")
  } else {
    df <- data.frame(Year = M$Yr,Catches=M$Obs_catch,type = "Observed")
    ggplot(df,aes(x=Year,y=Catches)) + geom_line(linewidth=2) + theme_few() + ylab("Catches (t)")
